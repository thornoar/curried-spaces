...
// + Докажем по индукции. База при $n=0$ очевидна, докажем переход $n-->n+1$. Рассмотрим произвольную $f in dd(n+1)$ и $x_0^((n+1))in RR^(n+1)$. Так как для всякого $x$ его образ $f(x) in dd(n)$, то для него выполняется $#v(10mm)tilde(f(x)) in C^1$. Тогда легко заметить, что, подставив $x_0^1$, мы получаем существование частных производных у $tilde(f)$ по всем координатам, кроме первой. Теперь заметим, что 
//   $
//     (partial tilde(f))/(partial x^1) (x_0)&= lim_(h->0) (tilde(f)(x_0^1+h,x_0^2,...,x_0^(n+1))-tilde(f)(x_0^1,x_0^2,...,x_0^(n+1)))/h\
//     &=lim_(h->0) (f(x_0^1+h)(x_0^2)...(x_0^(n+1))-f(x_0^1)(x_0^2)...(x_0^(n+1)))/h\
//     &= lim_(h->0) (h dot f'(x_0^1)(x_0^2)...(x_0^(n+1))+o(h))/h=f'(x_0^1)(x_0^2)...(x_0^(n+1))
//   $
//   Так как производная лежит в $cc(n+1)$, то есть непрерывна, то и частная производна тоже непрерывна. Значит, $tilde(f) in C^1$
// + Воспользуемся индукцией. База $n=0$ легко проверяется. Докажем переход. Возьмём произвольное $f in cc(n+1)$ такое, что $tilde(f) in C^1$. Рассмотрим $x_0^(n+1) in RR^(n+1)$ и $h in RR$. Тогда 
//   $
//     f(x_0^1+h)(x_0^2)...(x_0^(n+1))&=tilde(f)(x_0^1+h,x_0^2...,x_0^(n+1))\
//     &= tilde(f)(x_0^1,x_0^2...,x_0^(n+1))+sum_(i=1)^n h_i (partial tilde(f))/(partial x^i) (x_0^i) + o(h)\
//     &=f(x_0^1)...(x_0^(n+1))+(partial tilde(f))/(partial x^1) (x_0^1) +o(h) 
//   $
//   То есть
//   $
//     f(x_0^1+h)=f(x_0^1)+(partial tilde(f))/(partial x^1) (x_0^1)+o_1(h)
//   $
//   Значит, обозначив $f'(x_0^1):=display((partial tilde(f))/(partial x^1)) (x_0^1)$, мы получаем искомую непрерывную частную производную.


// #th[
//   Функция $f in HH_n (RR)$ принадлежит $dd(n)$ если и только если существует $lim_(h->0) display((f(x_0+h)-f(x_0))/h)$ для всех $x_0$, причём этот предел равен производной функции $f$ в точке $x_0$
// ]
// #proof([],[equiv])[

// + Заметим, что в топологических векторных пространствах справедливы арифметические действия с пределами. Тогда
//   $
//     limm(h,0) (f(x_0+h)-f(x_0))/h=limm(h,0) (h dot f'(x_0)+o(h))/h=f'(x_0)
//   $
//   Что и требовалось

// + Допустим $lim_(h->0) display((f(x_0+h)-f(x_0))/h)=A(x_0)in cc(n)$. Тогда 
//   $
//     0=limm(h,0)((f(x_0+h)-f(x_0))/h-A(x_0))=limm(h,0)((f(x_0+h)-f(x_0)-h dot A(x_0))/h)
//   $
//   Значит, 
//   $
//     f(x_0+h)-f(x_0)-h dot A(x_0)=o(h)\
//     f(x_0+h)=f(x_0)+h dot A(x_0)+o(h)
//   $
//   Следовательно, $f in dd(n)$
// ]

// / Следствие: Производная единственна
// #proof([],[])[
//   Следует из хаусдорфовости (второй аксиомы отделимости), так как в этом случае предел единственен
// ]
